...
import mklog from '../../app/util/log'
...
test('configure', (t) => {
  ...
  const stubs = {
    ...
    './configure/compute-size': test.module({
      computeFolderSize: async function () { return 0 }
    }),
    '../util/pathmaker': pathmaker
  }

  const configure = proxyquire('../../app/tasks/configure', stubs)
...
test('configure (each platform)', (t) => {
  const originalSf = require('../../app/util/sf').default
