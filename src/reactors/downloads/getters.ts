...
export function getPendingDownloads (downloads: IDownloadsState): IDownloadItem[] {
  const pending = filter(downloads.items, (i) => !i.finished);
  return sortBy(pending, "order");
}

export function getFinishedDownloads (downloads: IDownloadsState): IDownloadItem[] {
  const pending = filter(downloads.items, (i) => i.finished);
