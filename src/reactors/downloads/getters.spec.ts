...
suite(__filename, s => {
  ...
  s.case("getPendingDownloads", t => {
    ...
    t.same(getPendingDownloads({ items: activeAndInactive } as any), [
  ...
  s.case("getPendingForGame", t => {
    ...
    t.same(getPendingForGame({ items: oneActive } as any, 2121), [
      oneActive.disco,
    ]);
    t.same(getPendingForGame({ items: activeAndInactive } as any, 11), []);
    t.same(getPendingForGame({ items: activeAndInactive } as any, 22), [
      activeAndInactive.rock,
    ]);
  });
  ...
  s.case("excludeGame", t => {
    ...
    sameItems(excludeGame({ items: activeAndInactive } as any, -120), [
      activeAndInactive.rock,
      activeAndInactive.disco,
      activeAndInactive.punk,
    ]);
