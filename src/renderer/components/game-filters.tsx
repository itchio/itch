...
import { IRootState, TabLayout } from "common/types";
  ...
  render() {
    ...
    return (
      <FiltersContainer loading={loading}>
        ...
        {showBinaryFilters ? (
          <TagFilters>
            {T(["grid.criterion.filter_by"])}
            <div className="spacer" />
            {this.renderCriterion(
              "onlyCompatibleGames",
              T(["grid.filters.options.compatible"])
            )}
            {this.renderCriterion(
              "onlyOwnedGames",
              T(["grid.filters.options.owned"])
            )}
            {this.renderCriterion(
              "onlyInstalledGames",
              T(["grid.filters.options.installed"])
            )}
            <div className="spacer" />
        ...
        {numItems < 0 ? null : (
          <SecondaryText>
            {T(["grid.item_count", { count: numItems }])}
          </SecondaryText>
        )}
