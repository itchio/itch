...
module.exports = {
  ...
  package: async function(arch, buildPath) {
    ...
    await $.measure(
      `Writing installer to ${electronInstallerOpts.outputDirectory}`,
      async () => {
    ...
    $(
      await $.sh(
        `cp -vf ${installerPath}/${$.appName()}-${$.buildVersion()}*.nupkg ${installerPath}/*.exe ${installerPath}/RELEASES packages/`
      )
    );
  },
};
