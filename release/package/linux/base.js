...
module.exports = {
  prepareStage2: async function (buildPath, stage2Path) {
    ...
    $(await $.sh(`ln -s "../lib/${$.appName()}/${$.appName()}" "${stage2Path}/usr/bin/${$.appName()}"`))
    ...
    for (size of iconSizes) {
      ...
      $(await $.sh(`cp "release/images/${$.appName()}-icons/icon${size}.png" "${dir}/${$.appName()}.png"`))
    }

    $.say('Copying linux extras')
    $(await $.sh(`cp "linux-extras/io.itch.${$.appName()}.desktop" "${stage2Path}/usr/share/applications/io.itch.${$.appName()}.desktop"`))
    $(await $.sh(`cp "linux-extras/${$.appName()}.6" "${stage2Path}/usr/share/man/man6/${$.appName()}.6"`))
    $(await $.sh(`cp "linux-extras/${$.appName()}.appdata.xml" "${stage2Path}/usr/share/metainfo/${$.appName()}.appdata.xml"`))
  }
}
