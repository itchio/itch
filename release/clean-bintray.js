...
for (const repo of repos) {
  const listRes = $.get_output(`curl -u ${userkey} https://api.bintray.com/packages/itchio/${repo}/itch`)
  ...
  $.yesno(`delete ${deletedVersions.length} ${repo} versions? ${JSON.stringify(deletedVersions)}`)

  let i = 0
  for (const deletedVersion of deletedVersions) {
    $.say(`deleting v${deletedVersion} ${repo} (${deletedVersions.length - i} remaining)...`)
    $($.sh(`curl -u ${userkey} -X DELETE https://api.bintray.com/packages/itchio/${repo}/itch/versions/${deletedVersion}`))
    i++
  }
