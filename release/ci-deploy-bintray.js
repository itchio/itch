...
async function deployBintray () {
  ...
  var date = $.buildTime()
  const releaseDate = `${date.getUTCFullYear()}-${$.leftPad('' + date.getUTCMonth(), 2, '0')}-${$.leftPad('' + date.getUTCDate(), 2, '0')}`
  ...
  for (const repo of repos) {
    for (const arch of arches) {
      ...
      $(await $.qsh(`dpl --provider=bintray --file=build/bintray.json --user=fasterthanlime --key="${process.env.BINTRAY_TOKEN}"`))
    }
  }
}
