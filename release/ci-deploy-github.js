...
async function ciDeployGithub() {
  ...
  const rawTags = await $.getOutput('git for-each-ref --sort=taggerdate --format \'%(refname) %(taggerdate)\' refs/tags')
  ...
  const rawlog = await $.getOutput(`git log --oneline --no-merges ${previousTag}..${buildTag}`)
  ...
  $(await $.gothub(`release --tag ${buildTag} --draft --description "${changelog.replace(/`/g, '\\`')}"`))

  $.say('Uploading assets...')
  for (const name of (await $.findAllFiles('packages'))) {
    await $.retry(async function () {
      return await $.gothub(`upload --tag ${buildTag} --name ${ospath.basename(name)} --file ${name} --replace`)
    })
  }
}
